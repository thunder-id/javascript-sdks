export default function ThunderMark({
  height = 40,
  className,
  darkMode = false,
}: {
  height?: number
  className?: string
  darkMode?: boolean
}) {
  const width = Math.round((height / 257) * 207)

  return (
    <svg
      width={width}
      height={height}
      viewBox="0 0 207 257"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      aria-label="ThunderID"
    >
      <path
        d="M55.4763 26.4391L58.8866 0H0V26.4391H55.4763Z"
        fill={darkMode ? '#E8F4FF' : '#05213F'}
      />
      <path
        d="M39.8438 147.407L49.5455 72.2839H4.9909e-05V256.743H60.5602L80.048 147.407H39.8438Z"
        fill="#3688FF"
      />
      <path
        d="M192.42 59.361C182.782 40.2307 168.929 25.5705 150.903 15.3381C145.501 12.2662 139.761 9.6605 133.703 7.5208L115.401 103.702H159.757L76.2987 256.743H83.3735C109.449 256.743 131.69 251.574 150.14 241.236C168.569 230.897 182.634 216.131 192.356 196.959C202.058 177.765 206.909 154.8 206.909 128.043C206.909 101.286 202.079 78.5123 192.441 59.3821L192.42 59.361Z"
        fill="#3688FF"
      />
    </svg>
  )
}
