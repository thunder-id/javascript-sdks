# ThunderID SvelteKit Quickstart

A minimal SvelteKit application demonstrating ThunderID authentication with OAuth 2.0, PKCE, and JWT support using the `@thunderid/sveltekit` SDK.
...
1. Open `thunderid-config/thunderid.env` and set your preferred values:
   ```bash
   SVELTEKIT_QUICKSTART_CLIENT_ID=SVELTEKIT_QUICKSTART
   SVELTEKIT_QUICKSTART_REDIRECT_URIS=["http://localhost:5173"]
   ```
...
This creates the `Customer` user type and the `sveltekit-quickstart` application under the default organization unit.
...
2. Fill in your ThunderID credentials in `.env`, using the values you set in `thunderid-config/thunderid.env`:
   ...
   THUNDERID_CLIENT_ID=SVELTEKIT_QUICKSTART
   THUNDERID_CLIENT_SECRET=<the client secret>
