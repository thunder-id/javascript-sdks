...
import {EmbeddedFlowType, EmbeddedFlowExecuteRequestConfig, FlowExecutionError} from '../models/embedded-flow';
 ...
 * ```
 */
const executeEmbeddedUserOnboardingFlow = async ({
  url,
  baseUrl,
  payload,
  ...requestConfig
}: EmbeddedFlowExecuteRequestConfig): Promise<EmbeddedUserOnboardingFlowResponse> => {
  if (!payload) {
...
export default executeEmbeddedUserOnboardingFlow;
