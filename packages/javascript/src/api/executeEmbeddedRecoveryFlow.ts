...
import {EmbeddedFlowExecuteRequestConfig as EmbeddedFlowExecuteRequestConfig} from '../models/embedded-flow';
import {EmbeddedRecoveryFlowResponse} from '../models/embedded-recovery-flow';
 ...
 * ```
 */
const executeEmbeddedRecoveryFlow = async ({
  url,
  baseUrl,
  payload,
  ...requestConfig
}: EmbeddedFlowExecuteRequestConfig): Promise<EmbeddedRecoveryFlowResponse> => {
  if (!payload) {
...
export default executeEmbeddedRecoveryFlow;
