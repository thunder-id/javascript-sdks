...
const SENSITIVE_JSON_FIELD_PATTERN = /"(password|newPassword|currentPassword|secret|otp|pin|verificationCode|mfaToken)"\s*:\s*"[^"]+"/gi;
const SENSITIVE_PARAM_PATTERN = /(password|secret|otp|pin|code)=[^&\s]+/gi;
...
export function sanitizeForLog(input: string): string {
  ...
  result = result.replace(SENSITIVE_JSON_FIELD_PATTERN, (_match: string, key: string) => {
    return `"${key}":"***"`;
  });

  result = result.replace(SENSITIVE_PARAM_PATTERN, (match: string, key: string) => {
    return `${key}=***`;
  });
