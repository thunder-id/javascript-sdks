...
export interface BaseRecoveryProps {
  ...
  onComplete?: (response: EmbeddedRecoveryFlowResponse) => void;
  onError?: (error: Error) => void;
  onFlowChange?: (response: EmbeddedRecoveryFlowResponse) => void;
  onInitialize?: (payload?: EmbeddedRecoveryFlowRequest) => Promise<EmbeddedRecoveryFlowResponse>;
  onSubmit?: (payload: EmbeddedRecoveryFlowRequest) => Promise<EmbeddedRecoveryFlowResponse>;
  /**
...
const BaseRecoveryContent: FC<BaseRecoveryProps> = ({
  ...
  const [currentFlow, setCurrentFlow] = useState<EmbeddedRecoveryFlowResponse | null>(null);
  ...
  const normalizeFlowResponseLocal: any = useCallback(
    (response: EmbeddedRecoveryFlowResponse): EmbeddedRecoveryFlowResponse => {
  ...
  const handleSubmit = async (component: any, data?: Record<string, any>, skipValidation?: boolean): Promise<void> => {
    ...
    try {
      ...
      const payload: EmbeddedRecoveryFlowRequest = {
      ...
      if (response.flowStatus === EmbeddedRecoveryFlowStatus.Error) {
        handleError(response);
        onError?.(new Error(extractErrorMessage(response, t)));
        return;
      }

      if (response.flowStatus === EmbeddedRecoveryFlowStatus.Complete) {
        onComplete?.(response);
        return;
      }

      if (response.flowStatus === EmbeddedRecoveryFlowStatus.Incomplete) {
