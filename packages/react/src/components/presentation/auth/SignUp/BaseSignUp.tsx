...
export interface BaseSignUpProps {
   ...
   */
  onComplete?: (response: EmbeddedSignUpFlowResponse) => void;
   ...
   */
  onFlowChange?: (response: EmbeddedSignUpFlowResponse) => void;

  /**
   * Function to initialize sign-up flow.
   * @returns Promise resolving to the initial sign-up response.
   */
  onInitialize?: (payload?: EmbeddedSignUpFlowRequest) => Promise<EmbeddedSignUpFlowResponse>;
   ...
   */
  onSubmit?: (payload: EmbeddedSignUpFlowRequest) => Promise<EmbeddedSignUpFlowResponse>;
  /**
...
const BaseSignUpContent: FC<BaseSignUpProps> = ({
  ...
  const [currentFlow, setCurrentFlow] = useState<EmbeddedSignUpFlowResponse | null>(null);
  ...
  const normalizeFlowResponseLocal: any = useCallback(
    (response: EmbeddedSignUpFlowResponse): EmbeddedSignUpFlowResponse => {
      if (response?.data) {
  ...
  const setupFormFields: any = useCallback(
    (flowResponse: EmbeddedSignUpFlowResponse) => {
   ...
   */
  const isDisplayOnlyCompletion = (response: EmbeddedSignUpFlowResponse): boolean => {
   ...
   */
  const handleFlowCompletion = (response: EmbeddedSignUpFlowResponse): void => {
   ...
   */
  const handleRedirectionIfNeeded = (response: EmbeddedSignUpFlowResponse): boolean => {
    if (response?.type === EmbeddedSignUpFlowType.Redirection && (response?.data as any)?.redirectURL) {
      /**
  ...
  const handleSubmit = async (component: any, data?: Record<string, any>, skipValidation?: boolean): Promise<void> => {
    ...
    try {
      ...
      if (response.flowStatus === EmbeddedSignUpFlowStatus.Error) {
      ...
      if (response.flowStatus === EmbeddedSignUpFlowStatus.Complete) {
        handleFlowCompletion(response as EmbeddedSignUpFlowResponse);
        return;
      }

      if (response.flowStatus === EmbeddedSignUpFlowStatus.Incomplete) {
  ...
  useEffect(() => {
    ...
    const performPasskeyRegistration = async (): Promise<void> => {
      ...
      if (processedResponse.flowStatus === EmbeddedSignUpFlowStatus.Complete) {
        handleFlowCompletion(processedResponse as EmbeddedSignUpFlowResponse);
      } else {
